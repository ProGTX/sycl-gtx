...
cl_command_queue queue::create_queue(
  bool display_info,
...
// TODO: Specification requires const selector in queue and non-const in device
  : ctx(syclContext.get(), asyncHandler),
    ...
    command_q(create_queue()), command_group(this)
{
  command_q.release_one();
}
...
handler_event queue::process(buffer_set& buffers_in_use_master) {
  ...
  buffers_in_use_master.insert(
    command_group.write_buffers.begin(), command_group.write_buffers.end());
  is_flushed = true;
  return handler_event();
}

vector_class<cl_event> queue::get_wait_events(
