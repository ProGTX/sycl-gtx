...
device::device()
  : device(nullptr, detail::default_device_selector().get()) {}
...
static vector_class<device> create_sub_devices(
  ...
  auto error_code = clCreateSubDevices(
    did, properties, devices, device_ids, num_devices);
...
vector_class<device> detail::get_devices(
  ...
  cl_uint num_devices;
  auto error_code = clGetDeviceIDs(
    platform_id, device_type, MAX_DEVICES, device_ids, &num_devices);
  detail::error::report(error_code);
