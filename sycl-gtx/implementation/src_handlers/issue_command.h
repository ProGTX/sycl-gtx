...
class issue_command {
private:
	static void compile_command(queue* q, const vector_class<cl_event>& wait_events, kernel_::source src, shared_ptr_class<kernel> kern);
	static void prepare_kernel(shared_ptr_class<kernel> kern);

	static void enqueue_task_command(queue* q, const vector_class<cl_event>& wait_events, shared_ptr_class<kernel> kern, event* evnt);

	template <int dimensions>
	static void enqueue_range_command(
		queue* q, const vector_class<cl_event>& wait_events, shared_ptr_class<kernel> kern, event* evnt, range<dimensions> num_work_items, id<dimensions> offset
	) {
		prepare_kernel(kern);
	...
	static void enqueue_nd_range_command(
		queue* q, const vector_class<cl_event>& wait_events, shared_ptr_class<kernel> kern, event* evnt, nd_range<dimensions> execution_range
	) {
		prepare_kernel(kern);
