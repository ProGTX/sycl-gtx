...
class issue_command {
private:
	static void compile_command(queue* q, vector_class<cl_event>& wait_events, kernel_::source src, shared_ptr_class<kernel> kern);
	static void prepare_kernel(shared_ptr_class<kernel> kern);

	static void enqueue_task_command(queue* q, vector_class<cl_event>& wait_events, shared_ptr_class<kernel> kern, event* evnt);
