...
#include "accessor.h"
#include "ranges//id.h"
#include "../detail/src_handlers/register_resource.h"
...
private:
  ...
  using base_acc_device_ref = accessor_device_ref<
    dimensions, DataType, dimensions, mode, target>;
...
public:
  ...
  accessor_(
    cl::sycl::buffer<DataType, dimensions>& bufferRef, handler& commandGroupHandler)
    : accessor_(
      bufferRef,
      commandGroupHandler,
...
private:
  using subscript_return_t = typename subscript_helper<
    dimensions, DataType, dimensions, mode, target>::type;
public:
