...
struct Vec_ {
  type x, y, z;
  Vec_(const type& x_ = 0, const type& y_ = 0, const type& z_ = 0) {
    x = x_;
    y = y_;
    z = z_;
  }
  Vec_ operator+(const Vec_& b) const {
    return Vec_(x + b.x, y + b.y, z + b.z);
  }
  Vec_ operator-(const Vec_& b) const {
    return Vec_(x - b.x, y - b.y, z - b.z);
  }
  ...
  type dot(const Vec_& b) const {
    return x * b.x + y * b.y + z * b.z;
  }  // cross:
  Vec_ operator%(Vec_& b) {
    return Vec_(y * b.z - z * b.y, z * b.x - x * b.z, x * b.y - y * b.x);
  }
};
template <class type>
struct Ray_ {
  Vec_<type> o, d;
  Ray_(Vec_<type> o_, Vec_<type> d_) : o(o_), d(d_) {}
};
...
struct Sphere_ {
  ...
  Sphere_(const type& rad_, Vec_<type> p_, Vec_<type> e_, Vec_<type> c_,
          Refl_t refl_)
