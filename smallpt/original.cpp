    ...
    for(int sy = 0, i = (h - y - 1)*w + x; sy < 2; sy++)     // 2x2 subpixel rows
      for(int sx = 0; sx < 2; sx++, r = Vec()) {        // 2x2 subpixel cols
        for(int s = 0; s < samps; s++) {
          double r1 = 2 * get_random(Xi), dx = r1 < 1 ? sqrt(r1) - 1 : 1 - sqrt(2-r1);
          double r2 = 2 * get_random(Xi), dy = r2 < 1 ? sqrt(r2) - 1 : 1 - sqrt(2-r2);
...
void compute_org(
  void*, int w, int h, int samps, Ray cam, Vec cx, Vec cy, Vec r, Vec *c) {
  for(int y = 0; y < h; y++) { // Loop over image rows
    org::compute_inner(y, w, h, samps, cam, cx, cy, r, c);
  }
}
void compute_org_openmp(
  void*, int w, int h, int samps, Ray cam, Vec cx, Vec cy, Vec r, Vec *c) {
